...
fn test_round_trip_investigation_mixed_eol_and_cp1251() {
    ...
    // Исходный файл cp1251.txt содержит нетипичные переводы строк \r\r\n (0x0D 0x0D 0x0A).
    // Функция normalize_line_endings преобразует \r\r\n в \n\n (двойной перенос).
    // При кодировании обратно в CRLF каждый \n становится \r\n, давая \r\n\r\n (46 байт вместо 44).
    assert_eq!(cp1251_raw.len(), 44);
    assert_eq!(cp1251_encoded.len(), 46);
    // Первое расхождение на байте 25 (0x19): в исходном второй \r (0x0D), в результирующем \n (0x0A).
    assert_eq!(cp1251_raw[25], 0x0D);
    assert_eq!(cp1251_encoded[25], 0x0A);
}
