...
Единственное НЕТ — вариант `recentFiles` в окне настроек: списка недавних файлов в
программе нет. Настройка есть, и она ничего не делает; это решение владельца — убрать
вариант из интерфейса или завести список. Остальные ЧАСТИЧНО требуют приёмки руками.
...
| :--- | :---: | :--- |
| **1.1** Одно окно — один файл; нет хранилищ, боковой панели и вкладок | **ЕСТЬ** | Маршрутизация окон и отдельное пустое окно: `src-tauri/src/windows.rs:303-394`; оболочка редактора без вкладок/панелей: `src/App.svelte:747-844`. |
...
| `startupAction = recentFiles` | **НЕТ** | В UI значение предлагается (`src/ui/SettingsWindow.svelte:99-104`), но списка/загрузчика недавних файлов в `src/` и `src-tauri/src/` нет. Это известная незакрытая функция и выпускной блокер при сохранении этой опции. |
...
* НЕТ: 1 — `startupAction=recentFiles`; настройка показывается и ничего не делает, решение за владельцем, если она остаётся
  видимой. Решение: реализовать список или убрать вариант из Settings.
