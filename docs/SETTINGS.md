...
- отметки о применённых разовых переносах хранятся в объекте `migrations`
  (например, `migrations.fontFamilyDefault: true`): это позволяет один раз
  обновить устаревшие умолчания из старых сборок без перезаписи осознанного выбора.
...
проверяться не будет**. Список в настройках показывает, какие языки
доступны на этой машине, а не какие существуют вообще.

Настройки:

- проверка орфографии — включена или выключена;
- языки проверки. Здесь придётся оговориться честно: выбрать можно
  несколько, но проверяться будет первый выбранный. Редактор сообщает движку
  язык атрибутом `lang`, а тот по спецификации HTML принимает ровно одну
  метку языка: значение вида `en ru` недопустимо, и словарь не выберется
  вообще. Одновременную проверку на нескольких языках Chromium настраивает у
  себя в профиле, и со страницы до этой настройки не дотянуться. Список из
  нескольких языков имеет смысл оставить только как список, из которого
  переключаются;
